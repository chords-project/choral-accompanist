...
func getLocalIP(ctx context.Context) (string, error) {
	...
	if len(ips) == 0 {
		...
		if err != nil {
			slog.ErrorContext(ctx, fmt.Sprintf("An invalid network CIDR is set in environment DSB_HOTELRESERV_GRPC_NETWORK: %v", grpcNet))
		} else {
			for _, ip := range ips {
				if ipNetGrpc.Contains(ip) {
					ipGrpc = ip.String()
					slog.InfoContext(ctx, fmt.Sprintf("gRPC traffic is routed to the dedicated network %s", ipGrpc))
					break
				}
			}
		}
...
func (c *Client) Register(ctx context.Context, name string, id string, ip string, port int) error {
	if ip == "" {
		var err error
	...
	slog.InfoContext(ctx, "Trying to register service with registry", slog.String("name", name), slog.String("id", id), slog.String("address", fmt.Sprintf("%s:%d", ip, port)))
