...
func checkTLS() (bool, string) {
	if val, ok := os.LookupEnv("TLS"); ok {
		...
		if _, ok := cipherSuites[val]; ok {
			...
			slog.Warn("specified TLS cipher suite is invalid or unimplemented.", slog.String("cipher_suite", val))
			validCiphers := make([]string, 0, len(cipherSuites))
			for k := range cipherSuites {
				validCiphers = append(validCiphers, k)
			}
			slog.Warn("Please use the supported TLS cipher suite.", slog.Any("valid_ciphers", validCiphers))
			return false, ""
		}
	}
	return false, ""
...
func init() {
	...
	if needTLS {
		...
		if err != nil {
			log.Panicf("failed to read credentials: %v", err)
		}
		cp := x509.NewCertPool()
		if !cp.AppendCertsFromPEM(b) {
			log.Panicf("credentials: failed to append certificates")
		}
		config := tls.Config{
		...
		if cipher != "" {
			slog.Info("TLS enabled cipher suite %s", cipher)
			...
			slog.Info("TLS enabled without specified cipher suite")
		}
		...
		if err != nil {
			log.Panicf("failed to create credentials: %v", err)
		}
