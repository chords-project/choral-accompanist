{{- define "hotel-reservation.templates.baseDeploymentServices" }}
apiVersion: apps/v1
kind: Deployment
metadata:
  labels:
  ...
  template:
    ...
    spec:
      ...
      - name: "sidecar-{{ .Values.container.name }}"
        image: "{{ .Values.sidecar.dockerRegistry | default $.Values.global.dockerRegistry }}/{{ .Values.sidecar.image }}:{{ .imageVersion | default $.Values.global.defaultImageVersion }}"
