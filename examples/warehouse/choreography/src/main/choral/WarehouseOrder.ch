...
public class WarehouseOrder@(Warehouse, Payment, Loyalty) {
    ...
    public void orderFulfillment() {
        ctx_warehouse.transaction(svc_warehouse.checkItemInStockAndReserveForOrder());
        ch_warehousePayment.<Boolean>com(true@Warehouse);

        ctx_payment.transaction(svc_payment.takeMoneyFromCustomer());
        ch_paymentLoyalty.<Boolean>com(true@Payment);

        ctx_loyalty.transaction(svc_loyalty.awardPointsToCustomer());
        ch_loyaltyWarehouse.<Boolean>com(true@Loyalty);

        ctx_warehouse.transaction(svc_warehouse.packageAndSendOrder());
    }
}
