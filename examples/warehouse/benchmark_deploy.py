...
def discard_guard_command(context, namespace, run_id):
    """Print a scoped, ID-checked escape hatch for an intentionally discarded run."""
    return shlex.join(['kubectl', '--context', context, '-n', namespace,
                       'exec', 'deploy/loadgenerator', '--', 'sh', '-c', 'rm /runs/active-run'])


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('system', choices=STACKS)
    parser.add_argument('--benchmark', choices=('fault_tolerance', 'compensation'), default='fault_tolerance',
                        help='Locust benchmark to run (default: fault_tolerance)')
    ...
    if pvc and not existing:
        raise SystemExit(
            'Existing evidence PVC but no Locust deployment; restore the prior deployment and collect before switching')
    if existing:
        ...
        if guard:
            command = discard_guard_command(args.context, args.namespace, guard)
            raise SystemExit(
                f'Collect and drain run {guard} before deploying or switching stacks.\n'
                'If you have stopped Locust and intentionally want to discard this run, '
                f'remove its guard with:\n{command}'
            )
    ...
    with tempfile.TemporaryDirectory(prefix='warehouse-deploy-') as directory:
        ...
        rendered = subprocess.check_output(
            ['skaffold', 'render', *profile, *registry, '--digest-source',
             'remote' if args.default_repo else 'tag', '--build-artifacts', artifacts],
            cwd=ROOT, text=True,
        )
        rendered, replacements = re.subn(
            r'(?m)^([ \t]*- name: BENCHMARK_MODE\n[ \t]*value: )fault_tolerance$',
            lambda match: match.group(1) + args.benchmark, rendered,
        )
        if replacements != 1:
            raise RuntimeError('Cannot locate the loadgenerator benchmark mode in rendered manifests')
        ...
        if existing:
            ...
            subprocess.run([*kube, 'wait', '--for=delete', 'pod', '-l', 'app=loadgenerator', '--timeout=650s'],
                           check=True)
            ...
            check = {'apiVersion': 'v1', 'kind': 'Pod', 'metadata': {'name': 'benchmark-guard-check'}, 'spec': {
                'restartPolicy': 'Never', 'containers': [{'name': 'check', 'image': 'busybox:1.37.0',
                                                          'command': ['sh', '-c',
                                                                      'if [ -e /runs/active-run ]; then cat /runs/active-run; exit 1; fi'],
            ...
            try:
                ...
                while time.monotonic() < deadline:
                    phase = json.loads(output('get', 'pod', 'benchmark-guard-check', '-o', 'json')).get('status',
                                                                                                        {}).get('phase')
                    if phase == 'Succeeded': break
                    if phase == 'Failed': raise RuntimeError(
                        'A run started during deployment; restore Locust and collect it first')
                    time.sleep(1)
                else:
                    raise RuntimeError('Cannot verify benchmark guard')
            except Exception:
    ...
    print(f'{args.system} {args.benchmark} ready. Start the load-generator port forward with:\n{port_forward}')
